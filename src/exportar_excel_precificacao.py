...
def exportar_excel_precificacao(resultado: ResultadoPrecificacaoClasse, caminho_saida: str | Path) -> Path:
    """Exporta o resultado da Precificação Inteligente (abas Resumo,
    Cronograma Unificado e Memória de Cálculo) para um arquivo .xlsx."""
    caminho_saida = Path(caminho_saida)
    c = resultado.condicoes

    linhas = [
        _linha("Classe", resultado.classe),
        _linha("Valor Nominal do Crédito (C0)", formatar_moeda(float(resultado.valor_nominal_credito))),
        _linha(
        ...
        _linha("Taxa de Desconto (por período)", formatar_percentual(float(resultado.taxa_desconto_periodo))),
        ...
        _linha("Fluxo Nominal Total", formatar_moeda(float(resultado.fluxo_nominal_total))),
        _linha("Valor Presente do Fluxo (VP Total)", formatar_moeda(float(resultado.vp_total))),
        _linha("VPL Real Comercial", formatar_moeda(float(resultado.vpl_comercial))),
        _linha("Percentual de Recuperação Efetiva", formatar_percentual(float(resultado.percentual_recuperacao_efetiva) / 100)),
    ...
    with pd.ExcelWriter(caminho_saida, engine="openpyxl") as writer:
        ...
        df_fluxo.to_excel(writer, sheet_name="Cronograma Unificado", index=False)
        _aplicar_formatos(writer.sheets["Cronograma Unificado"], df_fluxo)
